...
pub enum Pane {
    Projects, // Top Left Sidebar: Projects List
    Files,    // Bottom Left Sidebar: Tracked Files in active project
    Main,     // Right Main Workspace
}

/// Active Mode for Main Workspace
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum MainViewMode {
    Inspector,   // Live Project Dashboard / Live File Inspector
    Explorer,    // Embedded File Explorer to add files
    HistoryDiff, // Commit History & Diffs & Restore
}

/// TUI mode (legacy tab alias)
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[allow(dead_code)]
pub enum Mode {
...
pub struct App {
    ...
    // NoteDog Sidebar Navigation State
...
impl App {
    ...
    pub fn refresh_projects(&mut self) {
        ...
        // Clamp active project index
        if !self.projects.is_empty() {
            if self.active_project_idx >= self.projects.len() {
                self.active_project_idx = self.projects.len().saturating_sub(1);
            }
            self.project_list_state.select(Some(self.active_project_idx));

            if let Some(proj) = self.projects.get(self.active_project_idx) {
                if !proj.files.is_empty() {
                    if self.active_file_idx >= proj.files.len() {
                        self.active_file_idx = proj.files.len().saturating_sub(1);
                    }
                    self.file_list_state.select(Some(self.active_file_idx));
                } else {
    ...
    pub fn active_project_name(&self) -> Option<String> {
        self.projects.get(self.active_project_idx).map(|p| p.name.clone())
    }

    /// Get the currently active file in the active project
    pub fn active_file(&self) -> Option<&DisplayFile> {
        self.projects
            .get(self.active_project_idx)
            .and_then(|p| p.files.get(self.active_file_idx))
    }

    /// Get the currently selected item (legacy compatibility)
    ...
    /// Get project name for current selection
    pub fn selected_project_name(&self) -> Option<String> {
        self.active_project_name()
    }

    /// Toggle expansion for selected project (legacy)
    ...
    /// Collapse selected project (legacy)
    ...
    /// Toggle encryption for the active file
    pub fn toggle_encryption(&mut self) {
        let (project_name, file_path) = if let Some(proj) = self.projects.get(self.active_project_idx) {
            if let Some(file) = proj.files.get(self.active_file_idx) {
                (proj.name.clone(), file.path.clone())
            } else {
    ...
    /// Toggle track mode for the active file (Git -> Backup -> Both -> Git)
    pub fn toggle_track_mode(&mut self) {
        ...
        if let Some(project) = self.manifest.get_project_mut(&project_name) {
            if let Some(file) = project.files.iter_mut().find(|f| f.path == file_path) {
                ...
                self.manifest_dirty = true;
                let mode_str = match file.track {
                    TrackMode::Git => "Git [G]",
                    TrackMode::Backup => "Backup [B]",
                    TrackMode::Both => "Both [+]",
                };
                self.message = Some((format!("Track mode set to {} (saves on exit)", mode_str), false));
                self.refresh_projects();
            }
        }
    }
    ...
    /// Switch focus to next pane (Projects -> Files -> Main -> Projects)
    pub fn next_pane(&mut self) {
        self.focused_pane = match self.focused_pane {
            Pane::Projects => Pane::Files,
            Pane::Files => Pane::Main,
            Pane::Main => Pane::Projects,
        };
        self.update_live_preview();
    }

    /// Switch focus to previous pane (Projects -> Main -> Files -> Projects)
    pub fn prev_pane(&mut self) {
        self.focused_pane = match self.focused_pane {
            Pane::Projects => Pane::Main,
            Pane::Files => Pane::Projects,
            Pane::Main => Pane::Files,
        };
        self.update_live_preview();
    }

    /// Move focus to right pane (Main)
    pub fn pane_right(&mut self) {
        self.focused_pane = Pane::Main;
        self.update_live_preview();
    }

    /// Move focus to left pane (Projects or Files)
    pub fn pane_left(&mut self) {
        if self.focused_pane == Pane::Main {
            self.focused_pane = Pane::Projects;
            self.update_live_preview();
        }
    }

    /// Open embedded file explorer in main pane to add files to active project
    pub fn open_explorer(&mut self) {
        self.main_view_mode = MainViewMode::Explorer;
        self.focused_pane = Pane::Main;
        if let Some(name) = self.active_project_name() {
            self.target_project = Some(name);
        }
    ...
    /// Open history, diff, and restore in main pane for active project
    pub fn open_history(&mut self) {
        self.main_view_mode = MainViewMode::HistoryDiff;
        self.focused_pane = Pane::Main;
        if let Some(name) = self.active_project_name() {
            self.load_commits_for_project(&name);
        }
    ...
    /// Close explorer/history and return to live inspector
    pub fn close_main_mode(&mut self) {
        self.main_view_mode = MainViewMode::Inspector;
        self.focused_pane = Pane::Projects;
        self.update_live_preview();
    }

    /// Navigate items down in the currently focused pane
    pub fn next_item(&mut self, step: usize) {
        match self.focused_pane {
            Pane::Projects => {
                if !self.projects.is_empty() {
                    let next = (self.active_project_idx + step).min(self.projects.len() - 1);
                    if next != self.active_project_idx {
                        self.active_project_idx = next;
                        self.active_file_idx = 0;
                        self.project_list_state.select(Some(self.active_project_idx));
            ...
            Pane::Files => {
                if let Some(proj) = self.projects.get(self.active_project_idx) {
                    if !proj.files.is_empty() {
                        let next = (self.active_file_idx + step).min(proj.files.len() - 1);
                        if next != self.active_file_idx {
                            self.active_file_idx = next;
                            self.file_list_state.select(Some(self.active_file_idx));
                            self.update_live_preview();
                        }
                    }
            ...
            Pane::Main => match self.main_view_mode {
                MainViewMode::Inspector => {
                    self.live_preview_scroll = self.live_preview_scroll.saturating_add(step);
                }
                MainViewMode::Explorer => {
                    self.browse_next(step);
                }
                MainViewMode::HistoryDiff => {
                    if self.restore_view == RestoreView::Commits {
                        if !self.commits.is_empty() {
                            let i = self.commit_list_state.selected().unwrap_or(0);
                            let next = (i + step).min(self.commits.len() - 1);
                            self.commit_list_state.select(Some(next));
                        }
                    } else if self.restore_view == RestoreView::Files {
                        if !self.restore_files.is_empty() {
                            let i = self.restore_list_state.selected().unwrap_or(0);
                            let next = (i + step).min(self.restore_files.len() - 1);
                            self.restore_list_state.select(Some(next));
                            self.update_live_preview();
                        }
                    }
                }
    ...
    /// Navigate items up in the currently focused pane
    pub fn prev_item(&mut self, step: usize) {
        match self.focused_pane {
            Pane::Projects => {
                if !self.projects.is_empty() {
                    let prev = self.active_project_idx.saturating_sub(step);
                    if prev != self.active_project_idx {
                        self.active_project_idx = prev;
                        self.active_file_idx = 0;
                        self.project_list_state.select(Some(self.active_project_idx));
                        self.file_list_state.select(Some(0));
                        self.update_live_preview();
            ...
            Pane::Files => {
                if let Some(proj) = self.projects.get(self.active_project_idx) {
                    if !proj.files.is_empty() {
                        let prev = self.active_file_idx.saturating_sub(step);
                        if prev != self.active_file_idx {
                            self.active_file_idx = prev;
                            self.file_list_state.select(Some(self.active_file_idx));
                            self.update_live_preview();
                        }
                    }
            ...
            Pane::Main => match self.main_view_mode {
                MainViewMode::Inspector => {
                    self.live_preview_scroll = self.live_preview_scroll.saturating_sub(step);
                }
                MainViewMode::Explorer => {
                    self.browse_prev(step);
                }
                MainViewMode::HistoryDiff => {
                    if self.restore_view == RestoreView::Commits {
                        if !self.commits.is_empty() {
                        ...
                        if !self.restore_files.is_empty() {
                            let i = self.restore_list_state.selected().unwrap_or(0);
                            let prev = i.saturating_sub(step);
                            self.restore_list_state.select(Some(prev));
                            self.update_live_preview();
                        }
                    }
                }
    ...
    pub fn bottom_item(&mut self) {
        match self.focused_pane {
            ...
            Pane::Files => {
                if let Some(proj) = self.projects.get(self.active_project_idx) {
                    if !proj.files.is_empty() {
                        self.active_file_idx = proj.files.len() - 1;
                        self.file_list_state.select(Some(self.active_file_idx));
                        self.update_live_preview();
                    }
                }
            ...
            Pane::Main => match self.main_view_mode {
                ...
                MainViewMode::HistoryDiff => {
                    if self.restore_view == RestoreView::Commits {
                        ...
                        if !self.restore_files.is_empty() {
                            self.restore_list_state.select(Some(self.restore_files.len() - 1));
                            self.update_live_preview();
                        }
                    }
                }
    ...
    /// Update the live preview cache for the right pane based on current mode and selection
    pub fn update_live_preview(&mut self) {
        ...
        match self.main_view_mode {
            MainViewMode::Inspector => {
                if let Some(proj) = self.projects.get(self.active_project_idx).cloned() {
                    if self.focused_pane == Pane::Files && !proj.files.is_empty() {
                        if let Some(file) = proj.files.get(self.active_file_idx) {
                            self.live_preview_title = file.path.clone();
                            self.live_preview_file_path = Some(file.abs_path.clone());
                            if file.abs_path.exists() {
                                if file.abs_path.is_file() {
                                    if let Ok(content) = fs::read_to_string(&file.abs_path) {
                                        let preview_str = content.lines().take(200).collect::<Vec<_>>().join("\n");
                                        self.live_preview_content = self.highlight_content(&preview_str, &file.abs_path);
                                    }
                                } else if file.abs_path.is_dir() {
                                    self.live_preview_content = self.load_folder_content(&file.abs_path);
                                }
                            }
                        }
                    } else {
                        // Project overview dashboard
                        self.live_preview_title = format!("Project: {}", proj.name);
                        if let Ok(project_dir) = self.config.project_dir(&proj.name) {
